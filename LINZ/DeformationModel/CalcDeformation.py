    ...
    try:
        for loop in [1]:
            ...
            if version not in model.versions():
                print(
                    "{0} is not a valid version of the deformation model".format(
                        version
                    )
                )
                break
            ...
            if not quiet:
                ...
                vsnopt = (
                    "between versions " + base_version + " and " + version
                    if base_version
                    else "for version " + version
                )
                datopt = (
                    "the date in column " + date_column if date_column else str(date)
                )
                if base_date:
            ...
            if atpoint:
                ...
                if update:
                    ...
                    print(
                        "{0:.{3}f} {1:.{3}f} {2:.{3}f}".format(
                            defm[0], defm[1], defm[2], ndp
                        )
                    )
                else:
                    print(
                        "Deformation at {0:.6f} {1:.6f}: {2:.{5}f} {3:.{5}f} {4:.{5}f}".format(
