...
class CsvFile(object):
        ...
        def __init__(self, name, typestr, optionalfield=False):
            ...
            def f(x):
                ...
                try:
                    ...
                    raise InvalidValueError(
                        "Cannot convert " + name + ' value "' + x + '" to ' + typestr
                    )
                return value
        ...
        def __init__(self, filetype, fields):
            ...
            for f in fields:
                try:
                    ...
                    if array:
                        init.append(
                            "  self." + fieldname + ".append(" + valuestr + ")\n"
                        )
                    else:
            ...
            exec(cdef, dict(fieldlist=fieldlist, container=self))
    ...
    def __init__(self, filetype, filename, fields):
        ...
        for i, field in enumerate(self._fields):
            if i + offset >= len(headers):
                missing = ", ".join(
                    [f.name() for f in self._fields[i:] if not f.optional()]
                )
                if missing:
        ...
        if message:
            raise ModelDefinitionError(
                "File "
                + filename
                + " does not have the correct columns for a "
                + filetype
                + " file\n"
                + message
            )
    ...
    def __next__(self):
        ...
        try:
            ...
            raise InvalidValueError(
                error + " in record " + str(self._nrec) + " of file " + self._filename
            )
