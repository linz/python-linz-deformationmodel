...
class Grid(object):
    ...
    def __init__(
        ...
        if self._nlon < 2 or self._nlat < 2:
            raise ModelDefinitionError(
                "Invalid number of grid rows or columns in deformation model definition for "
                + name
            )
        ...
        if self._dlon < 0:
            raise ModelDefinitionError(
                ...
                + " - "
                + str(maxlon)
                + " in deformation model definition for "
                + name
            )
    ...
    def load(self):
        ...
        try:
            ...
            if data:
                ...
                with CsvFile(
                    "grid", self._model.getFileName(self._gridfile), self._fields
                ) as f:
                    nc = -1
                    nr = 0
                    ...
                    for gpt in f:
                        ...
                        if nc >= self._nlon:
                            ...
                            if nr > self._nlat:
                                raise ModelDefinitionError(
                                    "Too many grid points in " + self._name
                                )
                        if (
                            math.fabs(gpt.lon - xc) > lontol
                            or math.fabs(gpt.lat - yc) > lattol
                        ):
                            raise ModelDefinitionError(
                                "Grid latitude/longitude out of sequence: ("
                                + str(gpt.lon)
                                + ","
                                + str(gpt.lat)
                                + ") should be ("
                    ...
                    self._valid = True
                    self._model.setCacheData(
                        self._data.data(), self._gridfile, gridmetadata
                    )
        finally:
            self._loaded = True
    ...
    def calcDeformation(self, x, y):
        ...
        if not self._valid:
            raise ModelDefinitionError(
                "Cannot use invalid grid component - see previous errors"
            )
        ...
        if x > self._maxlon or y < self._minlat or y > self._maxlat:
            raise OutOfRangeError(
                str(x0) + "," + str(y) + " is out of range of grid in " + self._name
            )
